...
public class Inventory : MonoBehaviour
{
    ...
    public bool AddItem(ItemData itemData)
    {
        ...
        if (items.Count >= maxSlots)
        {
            ...
            MessageLog.Instance?.AddMessage("¿Œ∫•≈‰∏Æ∞° ∞°µÊ √°Ω¿¥œ¥Ÿ");
            return false;
        }

        items.Add(itemData);
        DebugLog($"Added item: {itemData.itemName}");
        MessageLog.Instance?.AddMessage($"{itemData.itemName} »πµÊ");
        ...
        switch (item.itemType)
        {
            case ItemType.Consumable:
                if (health != null && item.healAmount > 0)
                {
                    ...
                    MessageLog.Instance?.AddMessage($"{item.itemName} ªÁøÎ");
                    RemoveItemAt(index);
                    return true;
                }
                break;
            ...
            case ItemType.Weapon:
                if (equipment != null)
                {
                    ...
                    MessageLog.Instance?.AddMessage($"{item.itemName} ¿Â¬¯");
                    RemoveItemAt(index);
                    return true;
                }
                break;
